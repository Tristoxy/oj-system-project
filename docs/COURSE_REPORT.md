...
并设置内存、CPU 和 PID 上限。动态语言命令不经过 shell，同时检查可执行程序白名单、危险
控制符和 `{src}`/`{exe}` 占位符。
...
分数和引用完整性，最后在一次锁内合并。任何检查失败都不会覆盖原文件；ID 冲突时新数据
覆盖旧数据，符合课程要求。

### 2.5 PDG 查重

Python 代码先解析 AST，以语句作为节点，建立顺序流、控制依赖和变量定义—使用的数据依赖；
变量名和常量被规范化。非 Python 代码使用规范化 token 图作为回退。相似度综合节点多重集合、
带端点标签的边集合和语句序列，并输出匹配节点及双方行号。PDG 随 submission 一起持久化，
管理员可以异步发起题目级两两比较并下载 JSON 报告。
...
本次生成版本的结果为 `38 passed`。Docker daemon 的实际运行结果和
