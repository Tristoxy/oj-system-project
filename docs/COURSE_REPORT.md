...
进阶部分按 2026 版要求实现了 AI 智能命题 R1–R4，并额外实现 Special Judge、Docker 沙箱
和基于程序依赖图的查重。
...
本次 Windows 开发环境的结果为 `68 passed, 1 skipped`；跳过项是仅能在 POSIX 系统验证的
进程组测试。课程不要求必须使用 Linux/WSL；如需额外验证 POSIX 进程组或 Docker daemon，
再在 Linux/WSL 或启用 Docker Desktop 的环境中补充即可。真实模型调用和 Streamlit 页面截图
仍需在本人电脑上人工验收，避免报告内容与现场演示不一致。
...
持久化队列，增加模型流式输出、自动测试数据验证、CSRF 防护和容器镜像漏洞扫描。
